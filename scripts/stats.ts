...
// Load a local .env for development if present. In CI there is no .env file —
// API_TOKEN_GITHUB is injected as a real environment variable instead.
try {
...
// devbyshima's account was created 2025-11-19 — no contributions exist before this.
export const START_DATE = new Date('2025-11-19T00:00:00.000Z');
...
async function getAllContributions(start: Date, end = new Date()) {
  ...
  return [years.reverse(), contributions] as const;
}
